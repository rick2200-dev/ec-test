...
until docker compose -f docker/docker-compose.deps.yaml exec -T postgres pg_isready -U ecmarket -d ecmarket_dev >/dev/null 2>&1; do
  sleep 1
done

echo "[4/5] Running database migrations..."
if command -v migrate >/dev/null 2>&1; then
  migrate -path db/migrations -database "postgres://ecmarket:localdev@localhost:5432/ecmarket_dev?sslmode=disable" up
else
...
PGPASSWORD=localdev psql -h localhost -U ecmarket -d ecmarket_dev -f db/seeds/dev_tenants.sql 2>/dev/null || echo "WARN: psql not found or seed failed. Run manually: make seed"
