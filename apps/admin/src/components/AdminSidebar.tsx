...
const navItems = [
  ...
  { href: "/tenants", label: "テナント管理", icon: "building" },
  { href: "/sellers", label: "セラー管理", icon: "users" },
  { href: "/commissions", label: "手数料設定", icon: "percent" },
  { href: "/analytics", label: "プラットフォーム分析", icon: "chart" },
  ...
  return (
    ...
    >
      <div className="h-16 flex items-center justify-between px-4 border-b border-primary-light">
        {!collapsed && (
          <span className="font-bold text-sm tracking-wide text-indigo-300">
            EC Marketplace
          </span>
        )}
        <button
      ...
      <div className="p-4 border-t border-primary-light">
        {!collapsed && (
          <p className="text-xs text-gray-500">Platform Admin v0.1</p>
        )}
      </div>
    </aside>
  );
