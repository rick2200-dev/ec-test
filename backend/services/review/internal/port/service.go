...
type ReviewUseCase interface {
	...
	CreateReview(ctx context.Context, buyerAuth0ID string, in domain.CreateReviewInput) (*domain.Review, error)
	// UpdateReview updates the caller's own review.
	UpdateReview(ctx context.Context, reviewID uuid.UUID, buyerAuth0ID string, in domain.UpdateReviewInput) (*domain.Review, error)
	// DeleteReview deletes the caller's own review.
	DeleteReview(ctx context.Context, reviewID uuid.UUID, buyerAuth0ID string) error
	...
	ListForSeller(ctx context.Context, sellerID uuid.UUID, limit, offset int) ([]domain.Review, int, error)
	// CreateReply creates a seller reply on a review.
	CreateReply(ctx context.Context, reviewID uuid.UUID, sellerAuth0ID string, sellerID uuid.UUID, in domain.CreateReplyInput) (*domain.ReviewReply, error)
	// UpdateReply updates a seller reply.
	UpdateReply(ctx context.Context, reviewID uuid.UUID, sellerAuth0ID string, sellerID uuid.UUID, in domain.UpdateReplyInput) (*domain.ReviewReply, error)
	// DeleteReply deletes a seller reply.
	DeleteReply(ctx context.Context, reviewID uuid.UUID, sellerID uuid.UUID) error
}
