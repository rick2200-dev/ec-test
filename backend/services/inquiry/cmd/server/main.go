...
func main() {
	...
	tShutdown, err := tracing.Init(context.Background(), tracing.LoadConfig("inquiry"))
	if err != nil {
	...
	sellerHandler := handler.NewSellerHandler(inquirySvc)
	...
	// Buyer-facing routes (tenant-scoped, mounted under /inquiries)
	r.Mount("/inquiries", buyerHandler.Routes())

	// Seller-facing routes (tenant + seller-scoped, mounted under /seller/inquiries)
	r.Mount("/seller/inquiries", sellerHandler.Routes())
	...
	go func() {
		slog.Info("starting inquiry service", "addr", addr)
	...
	slog.Info("inquiry service stopped")
}
