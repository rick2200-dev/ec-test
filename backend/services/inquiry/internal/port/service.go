...
type InquiryUseCase interface {
	...
	CreateInquiry(ctx context.Context, buyerAuth0ID string, in domain.CreateInquiryInput) (*domain.InquiryWithMessages, error)
	...
	PostMessage(ctx context.Context, actorAuth0ID string, actorSellerID *uuid.UUID, in domain.PostMessageInput) (*domain.InquiryMessage, error)
	// GetInquiry retrieves an inquiry with all messages, verifying that the actor is a participant.
	GetInquiry(ctx context.Context, inquiryID uuid.UUID, actorAuth0ID string, actorSellerID *uuid.UUID) (*domain.InquiryWithMessages, error)
	// ListForBuyer returns a paginated list of inquiries for the authenticated buyer.
	ListForBuyer(ctx context.Context, buyerAuth0ID string, limit, offset int) ([]domain.Inquiry, int, error)
	// ListForSeller returns a paginated list of inquiries for the given seller, optionally filtered by status.
	ListForSeller(ctx context.Context, sellerID uuid.UUID, status string, limit, offset int) ([]domain.Inquiry, int, error)
	// MarkRead records that the actor has read the inquiry's messages up to the current time.
	MarkRead(ctx context.Context, inquiryID uuid.UUID, readerType string, actorAuth0ID string, actorSellerID *uuid.UUID) error
	// CloseInquiry closes the inquiry; only the seller (actorSellerID non-nil) may perform this action.
	CloseInquiry(ctx context.Context, inquiryID uuid.UUID, actorSellerID *uuid.UUID) error
}
