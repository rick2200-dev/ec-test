...
type InquiryStore interface {
	...
	Create(ctx context.Context, inq *domain.Inquiry, msg *domain.InquiryMessage) (*domain.InquiryWithMessages, error)
	// GetByID retrieves an inquiry and all its messages by inquiry ID.
	GetByID(ctx context.Context, inquiryID uuid.UUID) (*domain.InquiryWithMessages, error)
	// ListByBuyer returns a paginated list of inquiries belonging to the buyer, ordered by latest activity.
	ListByBuyer(ctx context.Context, buyerAuth0ID string, limit, offset int) ([]domain.Inquiry, int, error)
	// ListBySeller returns a paginated list of inquiries directed at the seller, optionally filtered by status.
	ListBySeller(ctx context.Context, sellerID uuid.UUID, status string, limit, offset int) ([]domain.Inquiry, int, error)
	// AppendMessage adds a new message to an existing inquiry.
	AppendMessage(ctx context.Context, msg *domain.InquiryMessage) error
	// MarkRead marks messages as read by the specified reader type ("buyer" or "seller").
	MarkRead(ctx context.Context, inquiryID uuid.UUID, readerType string) error
	// Close sets the inquiry status to closed.
	Close(ctx context.Context, inquiryID uuid.UUID) error
}
