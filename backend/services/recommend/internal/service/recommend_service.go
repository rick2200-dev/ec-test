...
func NewRecommendService(eng engine.RecommendEngine, pool *pgxpool.Pool) *RecommendService {
	return &RecommendService{engine: eng, pool: pool}
}
...
func (s *RecommendService) RefreshPopularProducts(ctx context.Context) error {
	_, err := s.pool.Exec(ctx, "REFRESH MATERIALIZED VIEW CONCURRENTLY catalog_svc.popular_products")
	if err != nil {
