...
// GetBySKUID retrieves inventory for a specific SKU.
func (r *InventoryRepository) GetBySKUID(ctx context.Context, skuID uuid.UUID) (*domain.Inventory, error) {
	var inv domain.Inventory
	var found bool

	err := database.Tx(ctx, r.pool, func(tx pgx.Tx) error {
		row := tx.QueryRow(ctx,
			`SELECT id, sku_id, seller_id, quantity_available, quantity_reserved, low_stock_threshold, updated_at
			 FROM inventory_svc.inventory
			 WHERE sku_id = $1`, skuID)

		err := row.Scan(&inv.ID, &inv.SKUID, &inv.SellerID,
...
// List returns a paginated list of inventory records for a seller.
func (r *InventoryRepository) List(ctx context.Context, sellerID uuid.UUID, limit, offset int) ([]domain.Inventory, int, error) {
	...
	err := database.Tx(ctx, r.pool, func(tx pgx.Tx) error {
		// Count total.
		err := tx.QueryRow(ctx,
			`SELECT COUNT(*) FROM inventory_svc.inventory WHERE seller_id = $1`,
			sellerID,
		).Scan(&total)
		if err != nil {
		...
		for rows.Next() {
			var inv domain.Inventory
			if err := rows.Scan(&inv.ID, &inv.SKUID, &inv.SellerID,
...
func (r *InventoryRepository) Upsert(ctx context.Context, inv *domain.Inventory) error {
	return database.Tx(ctx, r.pool, func(tx pgx.Tx) error {
		...
		err := tx.QueryRow(ctx,
			`INSERT INTO inventory_svc.inventory (id, sku_id, seller_id, quantity_available, quantity_reserved, low_stock_threshold)
			 VALUES ($1, $2, $3, $4, $5, $6)
			 ON CONFLICT (sku_id)
			 DO UPDATE SET
...
func (r *InventoryRepository) Reserve(ctx context.Context, skuID uuid.UUID, quantity int) error {
	return database.Tx(ctx, r.pool, func(tx pgx.Tx) error {
		var available int
		err := tx.QueryRow(ctx,
...
func (r *InventoryRepository) Release(ctx context.Context, skuID uuid.UUID, quantity int) error {
	return database.Tx(ctx, r.pool, func(tx pgx.Tx) error {
		var reserved int
		err := tx.QueryRow(ctx,
...
func (r *InventoryRepository) ConfirmSold(ctx context.Context, skuID uuid.UUID, quantity int) error {
	return database.Tx(ctx, r.pool, func(tx pgx.Tx) error {
		var reserved int
		err := tx.QueryRow(ctx,
			`SELECT quantity_reserved FROM inventory_svc.inventory
...
func (r *InventoryRepository) AdjustStock(ctx context.Context, skuID uuid.UUID, quantity int) error {
	return database.Tx(ctx, r.pool, func(tx pgx.Tx) error {
		tag, err := tx.Exec(ctx,
			`UPDATE inventory_svc.inventory
			 SET quantity_available = quantity_available + $2,
...
func (r *InventoryRepository) RecordMovement(ctx context.Context, m *domain.StockMovement) error {
	return database.Tx(ctx, r.pool, func(tx pgx.Tx) error {
		m.ID = uuid.New()

		err := tx.QueryRow(ctx,
			`INSERT INTO inventory_svc.stock_movements (id, sku_id, movement_type, quantity, reference_type, reference_id)
			 VALUES ($1, $2, $3, $4, $5, $6)
			 RETURNING created_at`,
			m.ID, m.SKUID, m.MovementType, m.Quantity, m.ReferenceType, m.ReferenceID,
		).Scan(&m.CreatedAt)
		if err != nil {
...
func (r *InventoryRepository) ReleaseForOrderCancellation(
	...
	txErr := database.Tx(ctx, r.pool, func(tx pgx.Tx) error {
		...
		for _, line := range lines {
			...
			if _, err := tx.Exec(ctx,
				`INSERT INTO inventory_svc.stock_movements
				   (id, sku_id, movement_type, quantity, reference_type, reference_id)
				 VALUES ($1, $2, $3, $4, 'order_cancellation', $5)`,
				uuid.New(), line.SKUID, domain.MovementReleased, line.Quantity, orderID,
			); err != nil {
