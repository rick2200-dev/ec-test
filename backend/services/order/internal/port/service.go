...
type OrderUseCase interface {
	...
	CreateOrder(ctx context.Context, input domain.CreateOrderInput) (*domain.OrderWithLines, string, error)
	...
	CreateCheckout(ctx context.Context, input domain.CheckoutInput) (*domain.CheckoutResult, error)
	...
	GetOrder(ctx context.Context, orderID uuid.UUID) (*domain.OrderWithLines, error)
	// ListBuyerOrders returns a paginated list of orders placed by the buyer.
	ListBuyerOrders(ctx context.Context, buyerAuth0ID string, limit, offset int) ([]domain.Order, int, error)
	// ListSellerOrders returns a paginated list of orders received by the seller, optionally filtered by status.
	ListSellerOrders(ctx context.Context, sellerID uuid.UUID, status string, limit, offset int) ([]domain.Order, int, error)
	// UpdateOrderStatus updates the fulfillment status of an order; only the owning seller may perform this.
	UpdateOrderStatus(ctx context.Context, sellerID, orderID uuid.UUID, status string) error
	// ListPayouts returns a paginated list of seller payouts.
	ListPayouts(ctx context.Context, sellerID uuid.UUID, limit, offset int) ([]domain.Payout, int, error)
	// ListCommissionRules returns a paginated list of platform commission rules.
	ListCommissionRules(ctx context.Context, limit, offset int) ([]domain.CommissionRule, int, error)
	// CreateCommissionRule adds a new commission rule.
	CreateCommissionRule(ctx context.Context, rule *domain.CommissionRule) error
}
