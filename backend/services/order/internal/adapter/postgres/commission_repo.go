...
func (r *CommissionRepository) GetApplicableRule(ctx context.Context, sellerID uuid.UUID, categoryID *uuid.UUID) (*domain.CommissionRule, error) {
	var rule domain.CommissionRule
	var found bool

	err := database.Tx(ctx, r.pool, func(tx pgx.Tx) error {
		query := `SELECT id, seller_id, category_id, rate_bps, priority, valid_from, valid_until, created_at
		...
		err := tx.QueryRow(ctx, query, sellerID, categoryID).Scan(
			&rule.ID, &rule.SellerID, &rule.CategoryID,
...
// List returns a paginated list of commission rules.
func (r *CommissionRepository) List(ctx context.Context, limit, offset int) ([]domain.CommissionRule, int, error) {
	var rules []domain.CommissionRule
	var total int

	err := database.Tx(ctx, r.pool, func(tx pgx.Tx) error {
		if err := tx.QueryRow(ctx,
			`SELECT COUNT(*) FROM order_svc.commission_rules`,
		).Scan(&total); err != nil {
			return fmt.Errorf("count commission rules: %w", err)
		}

		rows, err := tx.Query(ctx,
			`SELECT id, seller_id, category_id, rate_bps, priority, valid_from, valid_until, created_at
			 FROM order_svc.commission_rules
...
func (r *CommissionRepository) Create(ctx context.Context, rule *domain.CommissionRule) error {
	rule.ID = uuid.New()

	return database.Tx(ctx, r.pool, func(tx pgx.Tx) error {
		return tx.QueryRow(ctx,
			`INSERT INTO order_svc.commission_rules
			 (id, seller_id, category_id, rate_bps, priority, valid_from, valid_until)
