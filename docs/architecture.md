...
└────────────────────────┬─────────────────────────────────┘
    ...
    ▼    ▼   ▼        ▼    ▼      ▼       ▼
┌─────┐┌─────┐┌──────┐┌─────┐┌─────┐┌──────┐┌──────┐
│Auth ││Cata-││Inven-││Order││Cart ││Search││Recom-│
│     ││log  ││tory  ││     ││(NEW)││      ││mend  │
│:8081││:8082││:8083 ││:8084││:8088││:8085 ││:8086 │
└──┬──┘└──┬──┘└──┬───┘└──┬──┘└──┬──┘└──┬───┘└──────┘
   │      │      │       │▲     │      │
   │      │      │       │└─────┤      │  ┌────────────┐
   │      │      │       │      │      │  │Notification│
...
**補足**: `Cart` サービスはチェックアウト時に内部 HTTP で `Order` サービスを呼び出す (図中の `Cart → Order` 矢印)。両サービスを跨る通信パターンの詳細は [通信パターン](#通信パターン) と [カート・チェックアウトと決済](#カートチェックアウトと決済) を参照。
...
サービス間の同期的なデータ取得には gRPC を使用します (今後実装予定)。現時点では HTTP/JSON による内部呼び出しも併用しています (例: cart → order)。
...
- 内部 HTTP エンドポイント (`/internal/*`) は Gateway を経由せず、クラスタ内部ネットワークからのみ到達可能
...
Amazon 型のカート UX では 1 つのチェックアウトで複数セラーの商品をまとめて決済します。本システムでは以下の不変条件でこれを表現します:

- **1 checkout = N orders (セラーごとに 1 行) が同一の `stripe_payment_intent_id` を共有する**
- `orders` テーブルには checkout を示す独立カラムを置かず、`stripe_payment_intent_id` をグルーピングキーとして利用
...
本システムは Amazon 型の**マルチセラーチェックアウト**をサポートします。購入者は複数セラーの商品を 1 つのカートに入れ、1 回の決済でまとめて購入できます。
...
Buyer frontend
    │
    │ 1. POST /api/v1/buyer/cart/items     (カート追加)
...
Gateway (:8080)  ──▶  Cart Service (:8088)
                          ...
                          └─ Order Service (内部 HTTP)
                                  │
                                  ├─ PostgreSQL  order_svc.orders (セラーごとに N 行, 同一 PI ID)
                                  ├─ PostgreSQL  order_svc.payouts (pending, セラーごとに N 行)
                                  └─ Stripe       CreatePaymentIntent (TransferData なし)

Stripe ──▶ POST /webhooks/stripe (order service)
              │
              └─ payment_intent.succeeded
                    ├─ PI ID で N 件の orders を取得
                    ├─ 各 order を paid に更新
                    ├─ 各 order について CreateTransfer (セラーの connected account へ)
                    └─ 対応する payouts 行を completed + transfer_id 付与
```
