...
import { getSellerCoupon, updateSellerCoupon } from "@/lib/api";
...
export default function EditSellerCouponPage({ params }: RouteProps) {
  const { id } = use(params);
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        const c = await getSellerCoupon(id);
        if (cancelled) return;
        setCoupon(c);
        setTitle(c.title);
