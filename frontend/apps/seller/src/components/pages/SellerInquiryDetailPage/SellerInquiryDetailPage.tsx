...
import { getSellerInquiry } from "@/lib/api";
...
export interface SellerInquiryDetailPageProps {
  id: string;
}

export default function SellerInquiryDetailPage({ id }: SellerInquiryDetailPageProps) {
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        const data = await getSellerInquiry(id);
